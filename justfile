...
# Compare formatter output without modifying files, matching the CI format gate.
format-check:
    #!/usr/bin/env bash
    set -euo pipefail
    while IFS= read -r -d '' file; do
        swift-format format --configuration .swift-format "$file" | diff -u "$file" -
    done < <(find ./Package.swift ./Sources ./Tests -type f -name '*.swift' -print0)
