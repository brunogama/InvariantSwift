...
/// Macro implementation for @Deterministic.
///
/// Generates a property test verifying f(x) == f(x) across multiple calls.
/// See DeterministicMacroDeclaration.swift for public API documentation.
// swiftlint:disable:next type_body_length
public struct DeterministicMacro: PeerMacro {
  ...
  private static func extractConfig(from node: AttributeSyntax) -> DeterministicConfig {
    var iterations = 100
    var callCount = 2

    guard case .argumentList(let args) = node.arguments else {
      return DeterministicConfig(iterations: iterations, callCount: callCount)
    }
  ...
  private static func buildWrapperEnum(
    ...
    let enumName = "\(funcDecl.name.text)_DeterministicTest"
  ...
  private static func buildTestClosure(
    ...
    // For simplicity with callCount=2, we do:
    // let call1 = f(input)
    // let call2 = f(input)
    // return call1 == call2
  ...
  private static func buildConfigDeclaration(config: DeterministicConfig) -> VariableDeclSyntax {
  ...
  private static func buildFailureCase() -> SwitchCaseSyntax {
    ...
    let issueRecord = buildIssueRecordCall("Determinism property violated")
...
private struct DeterministicConfig {
  let iterations: Int
  let callCount: Int
}
