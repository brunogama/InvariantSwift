...
private enum testUserValidation_PropertyTest {
    @Test("testUserValidation") static func run() throws {
        ...
        case .failure(counterexample: let counterexample, iterations: let iterations, shrunk: let shrunk, reason: _, seed: let seed):
            Issue.record(Comment(rawValue: "Property failed after \(iterations) iterations. Original: User's Age=\(counterexample), Account Balance=\(counterexample) | Shrunk: User's Age=\(shrunk), Account Balance=\(shrunk) | Seed: \(seed.rawValue)"))
