...
private enum testWithConfig_PropertyTest {
    @Test("testWithConfig") static func run() throws {
        ...
        case .failure(counterexample: let counterexample, iterations: let iterations, shrunk: let shrunk, reason: _, seed: let seed):
            Issue.record(Comment(rawValue: "Property failed after \(iterations) iterations. Original: x=\(counterexample), y=\(counterexample) | Shrunk: x=\(shrunk), y=\(shrunk) | Seed: \(seed.rawValue)"))
