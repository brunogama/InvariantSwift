...
private enum testWithConfig_PropertyTest {
    @Test("testWithConfig", InvariantSwiftPropertyExecutionTrait(testName: "testWithConfig", labels: ["x", "y"], configuredSeed: 42), .tags(.invariantSwiftPropertyBased)) static func run() throws {
        ...
        try executeGeneratedPropertyTest(property, config: config, testName: "testWithConfig", labels: ["x", "y"], persistFailures: false)
    }
}
