...
private enum testComplex_PropertyTest {
    @Test("testComplex", InvariantSwiftPropertyExecutionTrait(testName: "testComplex", labels: ["count", "User name"], configuredSeed: nil), .tags(.invariantSwiftPropertyBased)) static func run() throws {
        ...
        try executeGeneratedPropertyTest(property, config: config, testName: "testComplex", labels: ["count", "User name"], persistFailures: false)
    }
}
