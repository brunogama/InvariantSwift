...
private enum testComplex_PropertyTest {
    @Test("testComplex") static func run() throws {
        ...
        case .failure(counterexample: let counterexample, iterations: let iterations, shrunk: let shrunk, reason: _, seed: let seed):
            Issue.record(Comment(rawValue: "Property failed after \(iterations) iterations. Original: count=\(counterexample), User name=\(counterexample) | Shrunk: count=\(shrunk), User name=\(shrunk) | Seed: \(seed.rawValue)"))
