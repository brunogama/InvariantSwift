...
struct CoverageCompletionTests {
  ...
  func debugAssertionPathsPropertyValidation() throws {
    ...
    let debugProperty1 = Property<Int>(generator: Gen<Int>.int) { value in
      ...
      if value == Int.min {
        // This path might trigger debug assertions. Gen<Int>.int does emit
        // Int.min, so the branch runs; it must still hold for the test to pass.
        return value >= Int.min
      }
      return true
    }

    let debugProperty2 = Property<String>(generator: Gen<String>.string) { string in
  ...
  func exerciseAllPublicAPIMethods() async throws {
    ...
    let gen2 = Gen.oneOf([gen1, Gen<Int>.int])
    ...
    let prop1 = Property(generator: Gen<Int>.int, predicate: { $0 > 0 })
    let prop2 = Property(generator: Gen<Int>.int, predicate: { $0 % 2 == 0 })
    ...
    let budget = await collector.currentBudget()
    let biasedGen = Gen<Int>.int.biased(by: budget)
  ...
  func propertyConfigurationEdgeCases() throws {
    ...
    let simpleProperty = Property(generator: Gen<Bool>.bool, predicate: { _ in true })
