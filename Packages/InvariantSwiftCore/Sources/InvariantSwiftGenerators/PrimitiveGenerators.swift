  ...
  public static var int: Gen<Int> {
    Gen<Int>(
      ...
      shrink: Shrink { n in
        ...
        // `magnitude`, not `abs`: abs(Int.min) overflows and traps, and the
        // generator emits Int.min as an edge case, so shrinking a failure
        // found at Int.min would crash the whole test process.
        if n.magnitude > 1 {
          let half = n / 2
          if half != n && half != 0 {
            shrunk.append(half)
