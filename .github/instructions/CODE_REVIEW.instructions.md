...
description: "General code review checklist and merge-gate guidance for Swift changes"
---
...
- Macros and code generation changes: expansion fixtures + round-trip compilation tests.
- Generators and shrinking changes: distribution/coverage tests + shrinking minimality properties.
- Execution and crash isolation changes: deterministic replay tests + crash harness coverage.
- Swift Testing integration changes: end-to-end tests using `@Test` / `#expect` in root `Tests/`.
...
- All Swift macro code expansion must be validated using SwiftSyntax/Swift AST–based expansion tests.
