...
public struct Edge: Hashable, Codable, Sendable {
    ...
    public init(from: String, to: String, weight: Double? = nil, label: String? = nil, metadata: [String: String] = [:]) {
        self.from = from
        self.to = to
        self.weight = weight
        self.label = label
...
extension Gen where T == DirectedGraph {
            ...
            generate: { rng, size in
                ...
                if size.value <= 3 {
                    let edgeCases: [DirectedGraph] = [
                        ...
                        DirectedGraph(vertices: [Vertex(id: "v0"), Vertex(id: "v1")], 
                                     edges: [Edge(from: "v0", to: "v1")]) // Single edge
                    ]
                ...
                let edgeCount = Int.random(in: 0...min(maxEdges, vertexIds.count * vertexIds.count), using: &rng)
                var edges: Set<Edge> = []
                ...
                for _ in 0..<edgeCount {
                    ...
                    if !allowMultipleEdges && edges.contains(where: { $0.from == edge.from && $0.to == edge.to }) {
                        continue
                    }
    ...
    public static func tree(maxVertices: Int = 10) -> Gen<DirectedGraph> {
        Gen<DirectedGraph>(
            generate: { rng, size in
                let vertexCount = max(1, Int.random(in: 1...min(maxVertices, max(size.value, 1)), using: &rng))
        ...
        func schemaGenerator(depth: Int) -> Gen<JSONSchema> {
            Gen<JSONSchema>(
                generate: { rng, size in
                    ...
                    switch schemaType {
                    case .object:
                        let propertyCount = Int.random(in: 1...min(maxProperties, max(size.value, 1)), using: &rng)
                        var properties: [String: JSONSchema] = [:]
                        var required: [String] = []
                        
                        for i in 0..<propertyCount {
                            let propName = "prop\(i)"
                            properties[propName] = schemaGenerator(depth: depth + 1).generate(&rng, Size(size.value / 2))
                    ...
                    case .string:
                        let enumValues = Bool.random(using: &rng) ? 
                ...
                shrink: Shrink { schema in
                    ...
                    // Remove constraints
                    if schema.minimum != nil || schema.maximum != nil || schema.pattern != nil || schema.enum != nil {
                        shrunk.append(JSONSchema(type: schema.type, properties: schema.properties, items: schema.items))
                    }
...
public struct DatabaseRecord: Codable, Sendable, Equatable {
    ...
    public init(id: String, fields: [String: DatabaseValue], timestamp: Date = Date(), metadata: [String: String] = [:]) {
        self.id = id
        self.fields = fields
...
extension Gen where T == DatabaseRecord {
    ...
    public static func databaseRecord(
        maxFields: Int = 10,
        fieldNames: [String] = ["name", "email", "age", "active", "created", "score", "category", "description"]
    ) -> Gen<DatabaseRecord> {
        Gen<DatabaseRecord>(
            generate: { rng, size in
                ...
                for fieldName in selectedFields {
                    ...
                    case "name", "title", "category":
                        value = .string("Generated \(fieldName.capitalized) \(Int.random(in: 1...100, using: &rng))")
                    case "email":
                    ...
                    case "created", "updated", "timestamp":
                        let randomDate = Date().addingTimeInterval(Double.random(in: -86400*365...0, using: &rng))
...
public struct HTTPRequest: Codable, Sendable, Equatable {
    ...
    public init(method: HTTPMethod, path: String, headers: [String: String] = [:], body: Data? = nil, queryParameters: [String: String] = [:]) {
        self.method = method
        self.path = path
        ...
        Gen<HTTPRequest>(
            generate: { rng, size in
                ...
                let pathSegments = (0..<Int.random(in: 1...maxPathSegments, using: &rng)).map { i in
                    let resourceTypes = ["users", "posts", "orders", "products", "comments", "files", "api", "v1", "admin"]
                    if i % 2 == 0 {
                ...
                for paramName in commonParams.shuffled(using: &rng).prefix(paramCount) {
                    ...
                    case "sort":
                        queryParams[paramName] = ["asc", "desc", "created", "updated"].randomElement(using: &rng)!
                    case "format":
