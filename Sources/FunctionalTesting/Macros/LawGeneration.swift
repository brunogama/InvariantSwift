...
public macro FunctorLaws() = #externalMacro(module: "FunctionalTestingMacros", type: "FunctorLawsMacro")

/// Macro that generates applicative law tests for a type
@attached(member, names: arbitrary)
public macro ApplicativeLaws() = #externalMacro(module: "FunctionalTestingMacros", type: "ApplicativeLawsMacro")
...
@attached(member, names: arbitrary)
public macro CustomLaws() = #externalMacro(module: "FunctionalTestingMacros", type: "CustomLawsMacro")

/// Macro that generates property-based test laws from function signatures
@freestanding(expression)
public macro deriveLaw<T>(_ expression: T) -> Property<T> = #externalMacro(module: "FunctionalTestingMacros", type: "DeriveLawMacro")

/// Macro that generates comprehensive test suite for algebraic structures
@attached(member, names: arbitrary)
public macro AlgebraicLaws() = #externalMacro(module: "FunctionalTestingMacros", type: "AlgebraicLawsMacro")
...
public struct FunctorLawsMacro: MemberMacro {
    public static func expansion(
        ...
        guard let typeDecl = declaration.as(StructDeclSyntax.self) ??
                            declaration.as(ClassDeclSyntax.self) ??
                            declaration.as(EnumDeclSyntax.self) else {
            throw MacroExpansionError.invalidDeclaration("FunctorLaws can only be applied to struct, class, or enum")
        }
    ...
    private static func generateFunctorIdentityLaw(typeName: String, genericParams: [String]) -> DeclSyntax {
        let genericConstraints = genericParams.isEmpty ? "" : "<\(genericParams.joined(separator: ", "))>"
    ...
    private static func generateFunctorCompositionLaw(typeName: String, genericParams: [String]) -> DeclSyntax {
        let genericConstraints = genericParams.isEmpty ? "" : "<\(genericParams.joined(separator: ", "))>"
    ...
    private static func generateFunctorTestSuite(typeName: String, genericParams: [String]) -> DeclSyntax {
        let genericConstraints = genericParams.isEmpty ? "" : "<\(genericParams.joined(separator: ", "))>"
...
public struct ApplicativeLawsMacro: MemberMacro {
    public static func expansion(
        ...
        guard let typeDecl = declaration.as(StructDeclSyntax.self) ??
                            declaration.as(ClassDeclSyntax.self) ??
                            declaration.as(EnumDeclSyntax.self) else {
            throw MacroExpansionError.invalidDeclaration("ApplicativeLaws can only be applied to struct, class, or enum")
        }
    ...
    private static func generateApplicativeIdentityLaw(typeName: String, genericParams: [String]) -> DeclSyntax {
        let genericConstraints = genericParams.isEmpty ? "" : "<\(genericParams.joined(separator: ", "))>"
    ...
    private static func generateApplicativeCompositionLaw(typeName: String, genericParams: [String]) -> DeclSyntax {
        let genericConstraints = genericParams.isEmpty ? "" : "<\(genericParams.joined(separator: ", "))>"
    ...
    private static func generateApplicativeHomomorphismLaw(typeName: String, genericParams: [String]) -> DeclSyntax {
        let genericConstraints = genericParams.isEmpty ? "" : "<\(genericParams.joined(separator: ", "))>"
    ...
    private static func generateApplicativeInterchangeLaw(typeName: String, genericParams: [String]) -> DeclSyntax {
        let genericConstraints = genericParams.isEmpty ? "" : "<\(genericParams.joined(separator: ", "))>"
    ...
    private static func generateApplicativeTestSuite(typeName: String, genericParams: [String]) -> DeclSyntax {
        let genericConstraints = genericParams.isEmpty ? "" : "<\(genericParams.joined(separator: ", "))>"
...
public struct MonadLawsMacro: MemberMacro {
    public static func expansion(
        ...
        guard let typeDecl = declaration.as(StructDeclSyntax.self) ??
                            declaration.as(ClassDeclSyntax.self) ??
                            declaration.as(EnumDeclSyntax.self) else {
            throw MacroExpansionError.invalidDeclaration("MonadLaws can only be applied to struct, class, or enum")
        }
    ...
    private static func generateMonadLeftIdentityLaw(typeName: String, genericParams: [String]) -> DeclSyntax {
        let genericConstraints = genericParams.isEmpty ? "" : "<\(genericParams.joined(separator: ", "))>"
    ...
    private static func generateMonadRightIdentityLaw(typeName: String, genericParams: [String]) -> DeclSyntax {
        let genericConstraints = genericParams.isEmpty ? "" : "<\(genericParams.joined(separator: ", "))>"
    ...
    private static func generateMonadAssociativityLaw(typeName: String, genericParams: [String]) -> DeclSyntax {
        let genericConstraints = genericParams.isEmpty ? "" : "<\(genericParams.joined(separator: ", "))>"
    ...
    private static func generateMonadTestSuite(typeName: String, genericParams: [String]) -> DeclSyntax {
        let genericConstraints = genericParams.isEmpty ? "" : "<\(genericParams.joined(separator: ", "))>"
...
public struct CustomLawsMacro: MemberMacro {
    public static func expansion(
        ...
        guard let typeDecl = declaration.as(StructDeclSyntax.self) ??
                            declaration.as(ClassDeclSyntax.self) ??
                            declaration.as(EnumDeclSyntax.self) else {
            throw MacroExpansionError.invalidDeclaration("CustomLaws can only be applied to struct, class, or enum")
        }
    ...
    private static func generateCustomLaw(for method: MethodSignature, typeName: String) -> DeclSyntax? {
    ...
    private static func generateCustomTestSuite(typeName: String, methods: [MethodSignature]) -> DeclSyntax {
        let lawCalls = methods.map { "\($0.name)Law().contramap { _ in () as Any }" }.joined(separator: ",\n                ")
...
public struct AlgebraicLawsMacro: MemberMacro {
    public static func expansion(
        ...
        guard let typeDecl = declaration.as(StructDeclSyntax.self) ??
                            declaration.as(ClassDeclSyntax.self) ??
                            declaration.as(EnumDeclSyntax.self) else {
            throw MacroExpansionError.invalidDeclaration("AlgebraicLaws can only be applied to struct, class, or enum")
        }
    ...
    private static func generateAlgebraicTestSuite(typeName: String, conformances: [String]) -> DeclSyntax {
