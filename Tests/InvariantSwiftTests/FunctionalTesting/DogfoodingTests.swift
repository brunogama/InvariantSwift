...
struct DogfoodingTests {
  ...
  func shrinkingProducesSmallerValues() async {
    ...
    let gen = Gen<Int>.int(in: 10...1000)
  ...
  func genZipPreservesBothValues() async {
    ...
    let combined = Gen<Int>.int(in: 1...50).flatMap { a in
      Gen<Int>.int(in: 51...100).map { b in (a, b) }
    }
  ...
  func generatorFlatMapChaining() async {
    ...
    let chainedGen = Gen<Int>.int(in: 1...10).flatMap { _ in
      Gen<[Int]>.array(Gen<Int>.int(in: 0...100))
    }
  ...
  func propertyAndCombinesCorrectly() async {
    let positive = Property<Int>(generator: Gen<Int>.int(in: 1...100)) { $0 > 0 }
    let lessThan200 = Property<Int>(generator: Gen<Int>.int(in: 1...100)) { $0 < 200 }
