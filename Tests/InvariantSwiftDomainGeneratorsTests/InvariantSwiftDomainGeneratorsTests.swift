...
struct FakeryGeneratorsTests {
  ...
  func testFirstName() {
    ...
    let samples = (0..<100).map { gen.sample(size: Size(value: 10), seed: Seed(value: UInt64($0))) }
    #expect(!samples.isEmpty)
    // Edge cases can be empty, so we just check they are strings
    #expect(samples.allSatisfy { _ in true })
  }
  ...
  func testPrefix() {
    ...
    let samples = (0..<50).map { gen.sample(size: Size(value: 10), seed: Seed(value: UInt64($0))) }
    #expect(!samples.isEmpty)
  }
  ...
  func testZipCode() {
    ...
    let samples = (0..<100).map { gen.sample(size: Size(value: 10), seed: Seed(value: UInt64($0))) }
    let validSamples = samples.filter { $0.count == 5 && $0 != "ABCDE" }
  ...
  func testCoordinates() {
    ...
    #expect(
      samples.allSatisfy { lat, lon in
        if lat.isNaN || lat.isInfinite || lon.isNaN || lon.isInfinite { return true }
  ...
  func testUrl() {
    ...
    let samples = (0..<100).map { gen.sample(size: Size(value: 10), seed: Seed(value: UInt64($0))) }
    let validSamples = samples.filter { !$0.isEmpty && !$0.hasSuffix("://") && $0 != "not a url" }
    #expect(validSamples.allSatisfy { $0.contains("://") })
  }
  ...
  func testDomainName() {
    ...
    let samples = (0..<100).map { gen.sample(size: Size(value: 10), seed: Seed(value: UInt64($0))) }
    let validSamples = samples.filter { !$0.isEmpty && $0 != "example" }
    #expect(validSamples.allSatisfy { $0.contains(".") })
  }

  @Test("ipv4 generates valid IPv4 addresses")
  func testIpv4() {
    let gen = Gen.fake.internet.ipV4Address
    let samples = (0..<100).map { gen.sample(size: Size(value: 10), seed: Seed(value: UInt64($0))) }
    let validSamples = samples.filter {
      !$0.isEmpty && $0 != "0.0.0.0" && $0 != "999.999.999.999" && $0 != "a.b.c.d"
    }
    #expect(
  ...
  func testIpv6() {
    let gen = Gen.fake.internet.ipV6Address
    let samples = (0..<100).map { gen.sample(size: Size(value: 10), seed: Seed(value: UInt64($0))) }
    let validSamples = samples.filter {
      !$0.isEmpty && $0 != "::" && $0 != "::1" && $0 != "gggg:gggg:gggg:gggg:gggg:gggg:gggg:gggg"
    }
    #expect(
      validSamples.allSatisfy { ip in
        let parts = ip.split(separator: ":")
  ...
  func testEdgeCaseFrequency() {
    ...
    Gen.configureFake(edgeCaseFrequency: 1.0)
    let gen = Gen.fake.name.firstName
    let samples = (0..<20).map { gen.sample(size: Size(value: 10), seed: Seed(value: UInt64($0))) }

    // Should have high proportion of edge cases (empty, special chars, etc.)
    let edgeCases = samples.filter {
      $0.isEmpty || $0 == "A" || $0.contains("🙂") || $0 == "X Æ A-12" || $0 == "   "
    }
  ...
  func testCompositionMap() {
    ...
    let samples = (0..<10).map { gen.sample(size: Size(value: 10), seed: Seed(value: UInt64($0))) }
    let validSamples = samples.filter { !$0.isEmpty }
  ...
  func testDeterminism() {
    ...
    let sample1 = gen.sample(size: Size(value: 10), seed: seed)
    let sample2 = gen.sample(size: Size(value: 10), seed: seed)
  ...
  func testNonDeterminism() {
    ...
    let sample1 = gen.sample(size: Size(value: 10), seed: Seed(value: 1))
    let sample2 = gen.sample(size: Size(value: 10), seed: Seed(value: 2))
    let sample3 = gen.sample(size: Size(value: 10), seed: Seed(value: 3))
