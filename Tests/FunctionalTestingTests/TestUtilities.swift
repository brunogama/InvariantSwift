...
public struct TestUtilities {
    ...
    public static func expectSuccess<T>(
        ...
        case .success(let actualIterations):
            #expect(actualIterations == iterations, 
                   "Expected \(iterations) iterations, got \(actualIterations)")
        case .failure(let counterexample, let iter, let shrunk):
            Issue.record("Expected success but got failure: counterexample=\(counterexample), iterations=\(iter), shrunk=\(shrunk)")
        case .gaveUp(let discarded, let iter):
            Issue.record("Expected success but property gave up: discarded=\(discarded), iterations=\(iter)")
        }
    }
    ...
    public static func expectFailure<T: Comparable>(
        ...
        case .failure(let counterexample, let iterations, let shrunk):
            ...
            #expect(counterexamplePredicate(counterexample), 
                   "Counterexample \(counterexample) does not satisfy predicate")
            #expect(shrinkingPredicate(shrunk, counterexample),
        ...
        case .gaveUp(let discarded, let iterations):
            Issue.record("Expected failure but property gave up: discarded=\(discarded), iterations=\(iterations)")
        }
    }
    ...
    public static func expectGaveUp<T>(
        ...
        case .failure(let counterexample, let iterations, let shrunk):
            Issue.record("Expected gaveUp but got failure: counterexample=\(counterexample), iterations=\(iterations), shrunk=\(shrunk)")
        }
    }
    ...
    public static func validateGeneratorDiversity<T: Hashable>(
        ...
        #expect(uniqueValues.count >= minUniqueValues,
    ...
    public static func generateTestConfigurations() -> [PropertyConfig] {
        return [
            ...
            PropertyConfig(iterations: 1000, maxShrinks: 500, maxDiscarded: 1000, seed: Seed(value: 98765)),
    ...
    public static func runPropertiesConcurrently<T: Sendable>(
        ...
        return await withTaskGroup(of: PropertyResult<T>.self, returning: [PropertyResult<T>].self) { group in
    ...
    public static func analyzeResults<T>(
        ...
        let gaveUpCount = results.filter { if case .gaveUp = $0 { return true }; return false }.count
...
public struct ResultAnalysis {
    ...
    public var successRate: Double {
        return totalTests > 0 ? Double(successCount) / Double(totalTests) : 0.0
    }
    
    public var failureRate: Double {
        return totalTests > 0 ? Double(failureCount) / Double(totalTests) : 0.0
    }
    
    public var gaveUpRate: Double {
        return totalTests > 0 ? Double(gaveUpCount) / Double(totalTests) : 0.0
    }
}
...
private enum PropertyTestUtils {
    static func compareResults<T: Equatable>(_ lhs: PropertyResult<T>, _ rhs: PropertyResult<T>) -> Bool {
        switch (lhs, rhs) {
