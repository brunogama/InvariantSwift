...
struct LibFuzzerTests {
  ...
  @MainActor @Test("FuzzBridge with no target")
  ...
  @MainActor @Test("FuzzBridge with registered target")
  ...
  func testGenFuzz() {
    ...
    let value = Gen.int.fuzz(with: data)

    // Value should be deterministic
    let value2 = Gen.int.fuzz(with: data)
    #expect(value == value2)
  }

  @Test("Gen.fuzzTarget creates valid target")
  func testGenFuzzTarget() {
    let target = Gen.int.fuzzTarget(name: "IntProperty") { _ in true }
