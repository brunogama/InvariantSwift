  ...
  public static var shrink: Shrink<Config> {
    Shrink { value in
      ...
      for shrunkTimeout in Gen<Int>.int.shrink.shrink(value.timeout) {
        results.append(
          Config(timeout: shrunkTimeout, retries: value.retries, enabled: value.enabled)
        )
      }
      for shrunkRetries in Gen.optional(Gen<Int>.int).shrink.shrink(value.retries) {
        results.append(
          Config(timeout: value.timeout, retries: shrunkRetries, enabled: value.enabled)
        )
      }
      for shrunkEnabled in Gen<Bool>.bool.shrink.shrink(value.enabled) {
        results.append(
          Config(timeout: value.timeout, retries: value.retries, enabled: shrunkEnabled)
        )
      }
      return results
    }
