...
struct FloatingPointModeTests {
  ...
  func defaultDoubleFiniteOnly() {
    ...
    for i in 0..<1000 {
      let testSeed = Seed(value: seed.value + UInt64(i))
  ...
  func doubleShrinkTowardZero() async {
    ...
    switch result {
    case .failure(let counterexample, let shrunkValue, _, _, _):
      if let shrunk = shrunkValue {
        #expect(abs(shrunk) < abs(counterexample), "Shrunk value should have smaller magnitude")
